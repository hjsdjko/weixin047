...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusschongwushangpinCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusschongwushangpinCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discusschongwushangpin', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusschongwushangpin');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discusschongwushangpin');
			...
			async init(){
				...
				let res = await this.$api.info('discusschongwushangpin', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusschongwushangpin', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscusschongwushangpin/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
