...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view class="container" :style='{"width":"100%","padding":"0px","position":"relative","background":"#ffffff","height":"auto"}'>
				...
				<view class="detail-list-item" :style='{"margin":"12rpx 0 12rpx 0","borderColor":"#cccccc","borderWidth":"0 0 0px 0","background":"#ffffff","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
					<view class="lable" :style='{"width":"auto","padding":"0 20rpx 0 0","lineHeight":"48rpx","fontSize":"28rpx","color":"#333333","textAlign":"right"}'>店铺名称：</view>
					<view  class="text" :style='{"padding":"0px 0 0 0","margin":"0 0 0 0","lineHeight":"48rpx","fontSize":"28rpx","color":"#666666"}'>{{detail.dianpumingcheng}}</view>
				</view>
				<view class="detail-list-item" :style='{"margin":"12rpx 0 12rpx 0","borderColor":"#cccccc","borderWidth":"0 0 0px 0","background":"#ffffff","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			this.getStoreup();
			let cleanType = uni.getStorageSync('discusschongwushoumaiCleanType')
			if(cleanType){
				uni.removeStorageSync('discusschongwushoumaiCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('chongwushoumai', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','chongwushoumai');
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								...
								await _this.$api.update('chongwushoumai',_this.detail)
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','chongwushoumai');
			...
			async init(){
				...
				let res = await this.$api.info('chongwushoumai', this.id);
				this.detail = res.data;
				// 轮播图片
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discusschongwushoumai', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
            ...
            async onCartTap() {
				...
				await this.$api.add('cart', {
					...
					goodname: this.detail.chongwumingcheng,
					shangjiazhanghao: this.detail.shangjiazhanghao,
					goodtype: this.detail.diqu,
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discusschongwushoumai/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
