...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view :style='{"minHeight":"100vh","width":"100%","padding":"20rpx 0 0 0","position":"relative","background":"#ffffff","height":"100%"}'>
			<view :style='{"width":"calc(98% - 120rpx)","position":"relative","alignItems":"center","justifyContent":"space-between","display":"flex"}'>
				<view :style='{"backgroundColor":"#f5f5f5","margin":"0 30rpx 0 30rpx","color":"#333333","alignItems":"center","borderRadius":"40rpx","flex":"1","display":"flex","lineHeight":"64rpx","fontSize":"24rpx","height":"64rpx"}'>
					...
					<input v-model="searchForm.chongwumingcheng" type="text" placeholder="宠物名称" :style='{"background":"transparent","height":"100%"}'></input>
				</view>
			...
			<view class="list" :style='{"width":"100%","padding":"40rpx 24rpx 24rpx 24rpx","margin":"20rpx 0 0 0","flex":"1","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 0px 0px #00000020","padding":"0 0 20rpx 0","margin":"0 0 40rpx 0","borderColor":"#cccccc","flexWrap":"wrap","borderWidth":"0 0 2rpx 0","display":"flex","width":"100%","borderStyle":"dashed","justifyContent":"space-between","height":"auto"}' v-for="(product,index) in list" :key="index">
				  <image :style='{"margin":"0 20rpx 0 0","objectFit":"cover","borderRadius":"8rpx","display":"block","width":"200rpx","height":"200rpx","order":"2"}' mode="aspectFill" class="listmpic" v-if="preHttp(product.chongwutupian)" :src="product.chongwutupian.split(',')[0]"></image>
				  <image :style='{"margin":"0 20rpx 0 0","objectFit":"cover","borderRadius":"8rpx","display":"block","width":"200rpx","height":"200rpx","order":"2"}' mode="aspectFill" class="listmpic" v-else :src="product.chongwutupian?baseUrl+product.chongwutupian.split(',')[0]:''"></image>

				  <view class="listmr" :style='{"width":"calc(100% - 240rpx)","padding":"0 0 0 0","margin":"20rpx 0 0 0","height":"auto"}'>
					<view class="col3 f30 elip mb15" :style='{"padding":"0 20rpx 0 20rpx","margin":"0 0 12rpx","overflow":"hidden","whiteSpace":"nowrap","color":"#333","borderRadius":"8rpx","background":"#ffffff","width":"100%","lineHeight":"60rpx","fontSize":"28rpx","textOverflow":"ellipsis","height":"60rpx"}'>{{product.chongwumingcheng}}</view>
					<view class="col3 f30 elip mb15" :style='{"padding":"0 20rpx 0 20rpx","margin":"0 0 12rpx","overflow":"hidden","whiteSpace":"nowrap","color":"#333","borderRadius":"8rpx","background":"#ffffff","width":"100%","lineHeight":"60rpx","fontSize":"28rpx","textOverflow":"ellipsis","height":"60rpx"}'>宠物状态:{{product.chongwuzhuangtai}}</view>
					...
					<view :style='{"width":"100%","padding":"40rpx 20rpx 40rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto","order":"5"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('chongwushoumai','修改')) || (!userid && isAuthFront('chongwushoumai','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('chongwushoumai','删除')) || (!userid && isAuthFront('chongwushoumai','删除'))" @click.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"0","boxShadow":"0 0px 0px #cccccc","color":"#ffffff","bottom":"120rpx","right":"120rpx","outline":"none","borderRadius":"100%","background":"#3da742","width":"80rpx","lineHeight":"80rpx","fontSize":"26rpx","position":"fixed","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('chongwushoumai','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0","boxShadow":"0 0px 0px #cccccc","color":"#ffffff","bottom":"120rpx","right":"120rpx","outline":"none","borderRadius":"100%","background":"#3da742","width":"80rpx","lineHeight":"80rpx","fontSize":"26rpx","position":"fixed","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('chongwushoumai','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
	...
	<view :style='{"width":"80%","padding":"40rpx 20rpx","background":"#d1f1ed"}' class="screenBox" :class="screenBoxShow?'screenBoxActive':''">
		<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0"}'>
			<view :style='{"width":"100%","padding":"0 0 20rpx 20rpx","fontSize":"24rpx"}'>宠物状态</view>
			<view :style='{"width":"100%","flexWrap":"wrap","justifyContent":"space-around","display":"flex"}'>
				<view @click="chongwuzhuangtaiChange(item)" v-for="(item,index) in chongwuzhuangtaiOptions" :key="index" class="screenTab" :class="searchForm.chongwuzhuangtai==item?'screenTabActive':''">{{item}}</view>
			</view>
		</view>
		<view :style='{"width":"100%","padding":"20rpx 20rpx 20rpx 0","alignItems":"center","justifyContent":"space-around","display":"flex"}'>
			<view :style='{"width":"18%","padding":"0 0 0 20rpx","fontSize":"24rpx"}'>地区</view>
			<input :style='{"width":"80%","padding":"0 20rpx 0 20rpx","borderRadius":"40rpx","color":"#666666","background":"#ffffff","height":"60rpx"}' placeholder="请输入地区" v-model="searchForm.diqu">
		</view>
	...
	export default {
		...
		methods: {
			...
			chongwuzhuangtaiChange(e){
				this.searchForm.chongwuzhuangtai = e
				this.$forceUpdate()
			},
			screenReset(){
				this.searchForm = {}
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.chongwumingcheng){
					params['chongwumingcheng'] = '%' + this.searchForm.chongwumingcheng + '%'
				}
				if(this.searchForm.chongwuzhuangtai){
					params['chongwuzhuangtai'] = '%' + this.searchForm.chongwuzhuangtai + '%'
				}
				if(this.searchForm.diqu){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`chongwushoumai`, params);
                } else {
                    res = await this.$api.list(`chongwushoumai`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('chongwushoumai', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.chongwumingcheng){
					searchForm['chongwumingcheng'] = '%' + this.searchForm.chongwumingcheng + '%'
				}
				if(this.searchForm.chongwuzhuangtai){
					searchForm['chongwuzhuangtai'] = this.searchForm.chongwuzhuangtai
				}
				if(this.searchForm.diqu){
					searchForm['diqu'] = '%' + this.searchForm.diqu + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`chongwushoumai`, searchForm);
                } else {
                    res = await this.$api.list(`chongwushoumai`, searchForm);
                }
				// 如果是第一页数据置空
