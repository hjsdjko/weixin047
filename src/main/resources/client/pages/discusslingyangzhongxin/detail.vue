...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusslingyangzhongxinCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusslingyangzhongxinCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discusslingyangzhongxin', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusslingyangzhongxin');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discusslingyangzhongxin');
			...
			async init(){
				...
				let res = await this.$api.info('discusslingyangzhongxin', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusslingyangzhongxin', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscusslingyangzhongxin/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
