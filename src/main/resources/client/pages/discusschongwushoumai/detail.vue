...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusschongwushoumaiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusschongwushoumaiCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discusschongwushoumai', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusschongwushoumai');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discusschongwushoumai');
			...
			async init(){
				...
				let res = await this.$api.info('discusschongwushoumai', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusschongwushoumai', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscusschongwushoumai/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
