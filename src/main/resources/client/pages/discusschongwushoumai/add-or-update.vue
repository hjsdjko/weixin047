	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusschongwushoumai`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusschongwushoumai`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusschongwushoumai`, this.ruleForm);
						}else{
							await this.$api.add(`discusschongwushoumai`, this.ruleForm);
						}
						uni.setStorageSync('discusschongwushoumaiCleanType',true);
